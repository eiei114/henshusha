...
- **CI** runs on every PR and on pushes to `main` with a fast Ubuntu path for `pnpm typecheck`, `pnpm test:dependabot-config`, `pnpm test:dev-script-docs`, `pnpm test:ci-docs`, `pnpm test:examples`, `pnpm test:timeline-schema`, and `pnpm test:henshusha`.
